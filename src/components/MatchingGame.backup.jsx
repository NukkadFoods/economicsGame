...
const MatchingGame = ({ subject, onBackToHome }) => {
    ...
    const handleRestart = useCallback(() => {
        ...
        // Account for both scroll position and container position
        const scrollX = window.pageXOffset || document.documentElement.scrollLeft;
        const scrollY = window.pageYOffset || document.documentElement.scrollTop;

        // Calculate relative position to the container
        const x = fromType === 'option'
            ? dotRect.left + scrollX - containerRect.left + dotRect.width / 2
            : dotRect.right + scrollX - containerRect.left - dotRect.width / 2;
        const y = dotRect.top + scrollY - containerRect.top + dotRect.height / 2;
